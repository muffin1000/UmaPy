...
okeNameList = []
okeSaveName = []
vgmstream = "C:\\Users\\" + user + "\\vgmstream-win\\test.exe"
#oke
os.mkdir("./temp/oke")
for i in range(len(metaSql()[1])):
...
secondList = []
for i in range(len(masterSql()[1])):
    os.makedirs("./temp/song/" + masterSql()[1][i][1])
...
charaSongList.clear()
for i in range(num):
    charaSongList.append(["./temp/song/" + firstList[i] + "\\" + firstList[i] + secondList[i] + ".wav", songCopyPathList[i]])
    subprocess.run((vgmstream, "-o",  charaSongList[i][0], charaSongList[i][1]), shell=True)
    cmd = ["sox", "-m", "./temp/song/" + firstList[i] + "/" + firstList[i] + secondList[i] + ".wav", "./temp/oke/" + firstList[i] + ".wav", "./songs/" + firstList[i] + "/" + secondList[i] + ".wav"]
    subprocess.run(cmd)
    print("./temp/song/" + firstList[i] + "/" + firstList[i] + secondList[i] + ".wav")
    print("./temp/oke/" + firstList[i] + ".wav")
    print("./songs/" + firstList[i] + "/" + secondList[i] + ".wav")
shutil.rmtree("./temp")
for i in range(len(masterSql()[1])):
